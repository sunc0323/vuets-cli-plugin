...
module.exports = {
    ...
    // 替换文件中的内容
    fileStrReplace(filePath,files,oldStr,newStr){
      return new Promise(async resolve => {
        for (let i = 0; i<files.length; i++) {
          let data = fs.readFileSync(`${filePath}/${files[i]}`, 'utf8');
          let regp = new RegExp(`${oldStr}`, 'g');
          var result = data.replace(regp, newStr);
          // 加上
          ...
          fs.writeFileSync(`${filePath}/${files[i]}`, result, 'utf8');
        }
        return resolve(files);
      });
    }
