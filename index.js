...
program
  ...
  .option("-d,--dir <dir>", "指定创建的目录，不指定时则在项目src/views目录下创建页面或src/components目录下创建组件（执行命令目录路径下有这两个目录时），或者执行命令目录路径下创建页面或组件")
  .action((type,name,options) => {
    if(type=='page'){
      createPage(name,options.dir);
    }else if(type.indexOf('com')>-1){
      createComponent(name,options.dir);
    }
  });
/* = main entrance
-------------------------------------------------------------- */
